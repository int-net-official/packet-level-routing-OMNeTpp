...
void Ipv4::preroutingFinish(Packet *packet) {
    ...
    Ipv4Address un(Ipv4Address::UNSPECIFIED_ADDRESS);
    const auto &ipv4Header = packet->peekAtFront<Ipv4Header>();
    ASSERT(ipv4Header);
...
void Ipv4::routeUnicastPacket(Packet *packet) {
    ...
    if (destIE) {
        ...
        if (strstr(packet->getFullName(), "pfrp")) { // pfrp
            string modulePath = getParentModule()->getFullPath();
            const char *pktName = packet->getFullName();
            pair<string, int> p = pfrpTable::getInstance()->getRoute(modulePath, pktName, int(packet->getBitLength()));
            if (strstr(packet->getFullName(), "pfrpma")) {
                pfrpTable::getInstance()->countPktInNode(modulePath, pktName);
            }
            string s = p.first;
            InterfaceEntry *ie = ift->getInterface(p.second);
            L3Address next;
...
void Ipv4::reassembleAndDeliverFinish(Packet *packet) {
    ...
    for (const auto &elem : socketIdToSocketDescriptor) {
        if (elem.second->protocolId == protocol->getId() && (elem.second->localAddress.isUnspecified() || elem.second->localAddress == localAddress) && (elem.second->remoteAddress.isUnspecified() || elem.second->remoteAddress == remoteAddress)) {
            auto *packetCopy = packet->dup();
...
void Ipv4::fragmentAndSend(Packet *packet) {
    ...
    // pfrp survivalTime
    if (simTime().dbl() - packet->getCreationTime().dbl() >= survivalTime) {
        // drop datagram, destruction responsibility in ICMP
        PacketDropDetails details;
        details.setReason(HOP_LIMIT_REACHED);
        emit(packetDroppedSignal, packet, &details);
